...
public class TeamDetailsForMatchActivity extends AppCompatActivity {
        ...
        if (isInternetOn()){
            if(MainActivity.getUserPlayerObject().getType().equalsIgnoreCase("admin")) {
                if ( ! isSomeoneMissingInTheTeam()) {
                    if ( ! isSomeoneMissingInOpponentTeam()) {
                        ...
                        text.setTextSize(13);
                        text.setPadding(10, 10, 10, 10);
                        text.setText("Opponent Team Doesn't Have 5 members!");
                        toast.setView(text);
                        toast.show();
    ...
    protected Boolean isSomeoneMissingInOpponentTeam(){
        return (challengerTeam.getPlayer1().equalsIgnoreCase("no") ||
                challengerTeam.getPlayer2().equalsIgnoreCase("no") ||
                challengerTeam.getPlayer3().equalsIgnoreCase("no") ||
                challengerTeam.getPlayer4().equalsIgnoreCase("no") ||
                challengerTeam.getPlayer5().equalsIgnoreCase("no"));
    }
